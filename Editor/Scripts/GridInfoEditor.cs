    ...
    public class GridInfoEditor : Editor
    {
        ...
        private void DrawGrid(GridInfo script)
        {
            try
            {
                ...
                var coordinates = script.Coordinates.FindAll(pos => pos.x >= script.MaxSize.x || pos.y >= script.MaxSize.y);

                foreach (var c in coordinates)
                {
                    var index = script.Coordinates.FindIndex(pos => pos.x == c.x && pos.y == c.y);
